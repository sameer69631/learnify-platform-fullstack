  ...
  return (
    <div className='max-w-sm p-5 m-4 shadow-lg rounded-lg flex flex-col justify-center'>
      ...
      <div className='mt-1 mb-1'>
        <p className='flex items-center font-bold text-gray-800'>Bookings Between : {new Date(child.startDate).toLocaleDateString("en-GB")} - {new Date(child.endDate).toLocaleDateString("en-GB")}</p>
        <p className='flex items-center font-bold text-gray-800'>Timing : {new Date(child.startTime).toLocaleTimeString()} - {new Date(child.endTime).toLocaleTimeString()}</p>
      </div>
